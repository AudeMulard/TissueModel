...
os.chdir('../Data_1/Study_networks/continuous_limit_GN/')

def stress_strain_curve(test_number,network,type):
	import pandas as pd
	filenames = fnmatch.filter(os.listdir('.'), 'stress_data_*_%09d_3.rpt' % int(test_number))
	stress=[]
	lengths = []
...
complexity = [200,400,600,800,1200,1500,2000]
...
for type in ['global_length','number_bc','mean_length','number_ridge']:
	...
	for i in range(1,7):
		...
		color = np.random.rand(3,)
		#axss.plot(strain, mean_stress, label='%s' % (complexity[i]),color = color)
		#axss.plot(strain, stress_0, label='%s' % (len(network_0.vertices)))
		#axss.plot(strain, stress_1, label='%s' % (len(network_1.vertices)))
		#axss.plot(strain, stress_2, label='%s' % (len(network_2.vertices)))
		#axss.plot(strain, stress_3, label='%s' % (len(network_3.vertices)))
		#axss.plot(strain, stress_4, label='%s' % (len(network_4.vertices)))
		#plot_second_der(axssd,strain_0, stress_0,color = color)
		#axom.plot(strain_omega,omega_xx, label='%s, %s' % (creation, generation))
	axss.errorbar(mean_n_vertices,end_stress,end_std)
	if type == 'global_length':
		axss.set_ylabel(r'$\frac{1}{L}*\sum_{bn} f_k$ at 100% strain',fontsize=20)
	elif type == 'number_bc':
		axss.set_ylabel(r'$\frac{1}{n_{bn}}*\sum_{bn} f_k$ at 100% strain',fontsize=20)
	elif type == 'mean_length':
		axss.set_ylabel(r'$\frac{1}{l_0}*\sum_{bn} f_k$ at 100% strain',fontsize=20)
	elif type == 'number_ridge':
		axss.set_ylabel(r'$\frac{1}{l_0 * n_{ridges}}*\sum_{bn} f_k$ at 100% strain',fontsize=20)
	...
	fig_stress_strain.savefig('comp_networks_figure_stress_strain_%s_600.png' % type)
"""
########## ONE NETWORK ###########
for type in ['global_length','number_bc','mean_length','number_ridge']:
	end_stress = []
	...
	for i in range(8):
		...
		color = np.random.rand(3,)
		axss.plot(strain_0, stress_0, label='%s' % (complexity[i]),color = color)
		plot_second_der(axssd,strain_0, stress_0,color = color)
		#axom.plot(strain_omega,omega_xx, label='%s, %s' % (creation, generation))
	#axss.plot(strain_0,stress_0)
	if type == 'global_length':
