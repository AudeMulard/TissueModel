...
def set_boundary_conditions(network): ## can be changed zith the node label list
    ...
    filename = 'node_label_%09d_right.csv' % test_number
    with open(filename,'w') as writeFile:
        writer = csv.writer(writeFile,delimiter=',')
        writer.writerow(list_node_label_right)
    print(list_node_label_right)
    filename = 'node_label_%09d_left.csv' % test_number
    with open(filename,'w') as writeFile:
        writer = csv.writer(writeFile,delimiter=',')
        writer.writerow(list_node_label_left)
...
def define_mesh(mask):
	for i in range(len(ridge_vertices)):
		...
		myModel.rootAssembly.seedPartInstance(deviationFactor=0.1, 
			...
			mdb.models['Model-1'].rootAssembly.instances[instancename], ), size=0.01)
...
myModel.ConnectorSection(name='ConnSect-1', translationalType=
    JOIN)
