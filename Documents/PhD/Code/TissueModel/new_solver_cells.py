...
def set_boundary_conditions(network, side): ## can be changed with the node label list
    ## Get list of vertices on left and right boundaries
	list_vertices_right = []
	...
	## Get list of vertices on cell boundaries
	...
	if side=='right':
		...
		myModel.PinnedBC(createStepName='Initial', localCsys=None, name=
			'BC-1', region=Region(vertices=VertexArray(list_vertices_right)))
