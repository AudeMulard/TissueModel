...
class Cell:
	...
	def save_cell(self,path):
		filename = 'cell.csv'
		with open(os.path.join(path,filename), 'w') as writeFile:
			writer = csv.writer(writeFile)
			writer.writerow(["centre_x",self.centre_x])
			writer.writerow(["centre_y",self.centre_y])
			writer.writerow(["radius",self.radius])
			writer.writerow(["boundary",self.boundary_cell])
		writeFile.close()
